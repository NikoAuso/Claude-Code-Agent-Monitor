...
| `DASHBOARD_SSE_MAX_CLIENTS` | `50` | Maximum concurrent Server-Sent Events clients on `GET /api/events/stream`. Each open stream holds a socket for its lifetime, so the cap bounds what a runaway script can consume; further connections are refused with `503 STREAM_LIMIT` until one disconnects. Set to `0` to disable the SSE endpoint entirely (the WebSocket feed is unaffected) |
