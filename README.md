...
| `DASHBOARD_CODEX_HOOK_IDLE_MINUTES` | `10` | Idle window for retiring a **hook-only** Codex session — one whose run never wrote a rollout to disk (`codex exec --ephemeral`), so it has no transcript mtime to prove it is alive and no rollout for the process probe to match. `SessionEnd` normally closes it immediately; when that fire-and-forget hook is lost (dashboard down at exit, CLI killed) this silence window is the only remaining evidence, and it matters most on Windows where both process probes are unavailable. A session that is somehow still running self-heals on its next hook |
