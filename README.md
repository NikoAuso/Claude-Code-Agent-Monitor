...
| **Privacy (ingest redaction)**     | Configurable ingest-time privacy policy that redacts secrets from event payloads **before** they are written to SQLite. Built-in detectors cover secret-like keys (`token` / `password` / `api_key` / …), common API-key and Bearer / PEM patterns, optional email masking, and optional home-directory path hashing. Live hook processing still uses the raw in-memory payload (so sessions, agents, tokens, and costs keep working); only the persisted `events.data` blob is masked. The same policy applies to import/reimport. Manage toggles and preview sample payloads in **Settings → Privacy**, via `GET`/`PUT /api/settings/privacy` + `POST /api/settings/privacy/preview`, or `ccam privacy [show\|set]`. Defaults: secret key/value detectors ON; emails and home-path hashing OFF. Historical rows are not rewritten. |
| **Settings**                       | System info, hook status, model pricing management, notification preferences, **ingest privacy redaction**, data export **and restore** (the Import History panel's **Restore backup** mode re-imports a full export `.json` — idempotent and non-destructive, so you can consolidate several machines' history into one dashboard), session cleanup. The Model Pricing section exposes an info popover (the `i` icon next to the title) explaining how rule lookup works (first matching pattern wins), the SQL-style `%` wildcard syntax with concrete examples (`claude-opus-4-7%`, `claude-%-haiku`, exact ids), and that prices must be updated manually when Anthropic publishes new rates — already-stored sessions keep the price applied at ingest time. Each rule's editor also carries a collapsible **Introductory rates** block (a `YYYY-MM-DD` promo cutoff + per-category intro prices); leaving the date empty means no promo, and an empty date clears any stored intro rates. The CLAUDE_HOME box and Import History panel are fully i18n-driven across en/vi/zh |
...
ccam privacy                      # show ingest privacy redaction policy
ccam privacy set --emails         # update toggles (--enable/--disable, --secret-keys, --home-paths, …)
