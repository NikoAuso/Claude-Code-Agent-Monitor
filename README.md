...
| **Command Palette**              | Global `Cmd/Ctrl+K` launcher over pages, sessions, and quick actions. One query resolves three groups: the nine sidebar routes matched on their **translated** labels (so it works in every locale), live server-side session search via `/api/sessions?q=` (debounced, scope-aware, so it never holds a stale client-side index of thousands of sessions), and a short list of jumps that are otherwise several clicks deep. Also reachable from the search button at the top of the sidebar, which shows the platform-correct shortcut hint. Fully keyboard-driven — arrow keys, `Enter`, `Escape` — and degrades quietly if the session query fails |
