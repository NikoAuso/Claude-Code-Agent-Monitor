...
| `routes/settings.js`      | System info (DB size, hook status, server uptime, transcript cache stats), ingest privacy policy (`GET`/`PUT /api/settings/privacy`, `POST /api/settings/privacy/preview`), data export as one versioned JSON bundle and matching import/restore (`POST /api/settings/import` via `server/lib/data-transfer.js` — idempotent, session-atomic, non-destructive; consolidates machines), session cleanup (abandon stale, purge old), clear all data (including the fired-alert feed and webhook delivery log; alert *rules*, webhook *targets*, and *privacy_settings* are preserved as user configuration), reset pricing, reinstall hooks                                                                                                                                                                                                                                                                                                                                           |
| `lib/privacy.js`          | Ingest-time privacy redaction. Built-in detectors for secret-like keys, API-key / Bearer / PEM / Slack / GitHub / AWS patterns, optional emails, and optional home-directory path hashing. `redactForStorage` / `serializeEventData` are applied in `routes/hooks.js` (and import) **only** to the clone written to `events.data` — the in-memory hook payload stays intact so session/agent/token extraction is unaffected. Fail-safe: redactor errors never throw out of the hook path. Settings live in the single-row `privacy_settings` table |
