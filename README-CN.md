...
| `DASHBOARD_CODEX_HOOK_IDLE_MINUTES` | `10` | 结束**仅靠 hook** 的 Codex 会话所需的静默窗口——这类会话运行时未将 rollout 写入磁盘（`codex exec --ephemeral`），因此既没有 transcript mtime 可证明其存活，也没有 rollout 供进程探测匹配。`SessionEnd` 通常会立即关闭它；当这个 fire-and-forget hook 丢失时（退出时 dashboard 未运行、CLI 被强制终止），该静默窗口便是仅存的证据，在两种进程探测均不可用的 Windows 上尤为重要。若会话其实仍在运行，下一个 hook 会自动修复 |
