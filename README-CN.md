...
| `DASHBOARD_SSE_MAX_CLIENTS` | `50` | `GET /api/events/stream` 上同时连接的 Server-Sent Events 客户端上限。每个打开的流在其生命周期内都会占用一个套接字，因此该上限可限制失控脚本的资源消耗；超出的连接会以 `503 STREAM_LIMIT` 被拒绝，直到有连接断开。设为 `0` 可完全禁用 SSE 端点（WebSocket 推送不受影响）。 |
