...
| **命令面板** | 覆盖页面、会话与快捷操作的全局 `Cmd/Ctrl+K` 启动器。一次输入同时解析三组结果：按**已翻译**标签匹配的九个侧边栏路由（因此在任何语言下都可用）、经由 `/api/sessions?q=` 的服务端实时会话搜索（带防抖、遵循数据范围，因此不会在客户端保留数千条会话的陈旧索引），以及一小组原本需要点击多次才能到达的跳转。也可从侧边栏顶部的搜索按钮打开，该按钮会显示与平台匹配的快捷键提示。完全键盘驱动 —— 方向键、`Enter`、`Escape` —— 且在会话查询失败时安静降级 |
