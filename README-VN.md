...
| **Bảng lệnh** | Trình khởi chạy toàn cục `Cmd/Ctrl+K` cho trang, phiên và các hành động nhanh. Một truy vấn phân giải ba nhóm: chín tuyến trong sidebar khớp theo nhãn **đã dịch** (nên hoạt động ở mọi ngôn ngữ), tìm kiếm phiên phía máy chủ theo thời gian thực qua `/api/sessions?q=` (có debounce, tôn trọng phạm vi dữ liệu, nên không giữ chỉ mục cũ của hàng nghìn phiên phía client), và một danh sách ngắn các bước nhảy vốn cách xa vài lần nhấp. Cũng mở được từ nút tìm kiếm ở đầu sidebar, nơi hiển thị gợi ý phím tắt đúng theo nền tảng. Điều khiển hoàn toàn bằng bàn phím — phím mũi tên, `Enter`, `Escape` — và giảm cấp êm ái nếu truy vấn phiên thất bại |
