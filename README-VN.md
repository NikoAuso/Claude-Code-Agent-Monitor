...
| `DASHBOARD_CODEX_HOOK_IDLE_MINUTES` | `10` | Khoảng thời gian im lặng trước khi kết thúc một phiên Codex **chỉ-từ-hook** — phiên chạy mà không ghi rollout xuống đĩa (`codex exec --ephemeral`), nên không có mtime transcript để chứng minh nó còn sống và không có rollout cho process probe đối chiếu. `SessionEnd` thường đóng nó ngay lập tức; khi hook fire-and-forget đó bị mất (dashboard tắt lúc thoát, CLI bị kill) thì khoảng im lặng này là bằng chứng duy nhất còn lại, và điều này quan trọng nhất trên Windows nơi cả hai process probe đều không khả dụng. Phiên nào vẫn còn chạy sẽ tự phục hồi ở hook kế tiếp |
