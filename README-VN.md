...
| `DASHBOARD_SSE_MAX_CLIENTS` | `50` | Số lượng client Server-Sent Events đồng thời tối đa trên `GET /api/events/stream`. Mỗi luồng đang mở giữ một socket suốt vòng đời của nó, nên giới hạn này chặn mức tiêu thụ của một script chạy loạn; các kết nối vượt quá bị từ chối với `503 STREAM_LIMIT` cho đến khi có một luồng ngắt. Đặt `0` để tắt hoàn toàn endpoint SSE (feed WebSocket không bị ảnh hưởng) |
