...
| **명령 팔레트** | 페이지, 세션, 빠른 작업을 아우르는 전역 `Cmd/Ctrl+K` 실행기. 한 번의 입력으로 세 그룹을 해결합니다 — **번역된** 라벨로 매칭되는 아홉 개의 사이드바 라우트(따라서 모든 로케일에서 동작), `/api/sessions?q=`를 통한 서버 측 실시간 세션 검색(디바운스 적용, 데이터 범위를 존중하므로 수천 개 세션의 오래된 클라이언트 인덱스를 들고 있지 않음), 그리고 평소라면 여러 번 클릭해야 닿는 이동들의 짧은 목록. 사이드바 상단의 검색 버튼으로도 열 수 있으며, 이 버튼은 플랫폼에 맞는 단축키 힌트를 표시합니다. 방향키, `Enter`, `Escape`로 완전히 키보드 조작이 가능하고, 세션 검색이 실패해도 조용히 저하됩니다 |
