...
| `DASHBOARD_CODEX_HOOK_IDLE_MINUTES` | `10` | **hook 전용** Codex 세션을 종료하기까지의 유휴 시간 — rollout을 디스크에 기록하지 않고 실행된 세션(`codex exec --ephemeral`)이라 살아 있음을 증명할 transcript mtime도, 프로세스 프로브가 대조할 rollout도 없습니다. 보통은 `SessionEnd`가 즉시 닫지만, 그 fire-and-forget hook이 유실되면(종료 시 dashboard가 꺼져 있거나 CLI가 강제 종료된 경우) 이 침묵 구간이 유일한 근거가 되며, 두 프로세스 프로브가 모두 불가능한 Windows에서 특히 중요합니다. 실제로 아직 실행 중인 세션은 다음 hook에서 자동 복구됩니다 |
