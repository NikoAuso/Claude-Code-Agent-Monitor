...
window.__WIKI_CONTENT_I18N = {
  zh: {
    ...
    "Model pricing editor with per-token rate configuration for every Claude variant. Ingest privacy redaction masks secrets in event payloads before SQLite writes (Settings → Privacy, with a before/after preview). Hook installation status with one-click reinstall and per-hook health checks. Full JSON data export covering sessions, agents, events, tokens, and pricing rules. Session cleanup controls to abandon stale sessions or purge old data by age. Browser notification preferences with per-event toggles. A system information panel shows database row counts, file sizes, server uptime, and WebSocket connection status at a glance.":
      "模型定价编辑器，可为每个 Claude 变体配置按 token 计的费率。摄入隐私脱敏会在写入 SQLite 之前遮蔽事件载荷中的密钥（Settings → Privacy，含前后对比预览）。Hook 安装状态，支持一键重装以及对每个 hook 的健康检查。完整的 JSON 数据导出，涵盖会话、智能体、事件、token 和定价规则。会话清理控件，可放弃陈旧会话或按时长清除旧数据。浏览器通知偏好，支持按事件切换。系统信息面板可一目了然地显示数据库行数、文件大小、服务器运行时间以及 WebSocket 连接状态。",
    ...
    '<span class="caption-icon">⚙️</span> <span><strong>Settings</strong> — model pricing editor with per-token rates, ingest privacy redaction, hook installation status, JSON data export, session cleanup controls, browser notification toggles, and system info panel with DB stats</span>':
      '<span class="caption-icon">⚙️</span> <span><strong>Settings</strong> — 模型定价编辑器（含每 token 费率）、摄入隐私脱敏、hook 安装状态、JSON 数据导出、会话清理控制、浏览器通知开关，以及含数据库统计的系统信息面板</span>',
  ...
  vi: {
    ...
    '<span class="caption-icon">⚙️</span> <span><strong>Settings</strong> — model pricing editor with per-token rates, ingest privacy redaction, hook installation status, JSON data export, session cleanup controls, browser notification toggles, and system info panel with DB stats</span>':
      '<span class="caption-icon">⚙️</span> <span><strong>Settings</strong> — trình chỉnh sửa giá mô hình với mức phí theo từng token, che dữ liệu riêng tư khi thu nhận, trạng thái cài đặt hook, xuất dữ liệu JSON, các điều khiển dọn dẹp phiên, công tắc thông báo trình duyệt, và bảng thông tin hệ thống kèm thống kê DB</span>',
    "This chart tracks how interest in Claude Code Agent Monitor has grown over time. The curve keeps climbing as more developers discover the project, share it, and use it in real workflows. Each new star is a small vote of confidence from the community.":
  ...
  ko: {
    ...
    "Model pricing editor with per-token rate configuration for every Claude variant. Ingest privacy redaction masks secrets in event payloads before SQLite writes (Settings → Privacy, with a before/after preview). Hook installation status with one-click reinstall and per-hook health checks. Full JSON data export covering sessions, agents, events, tokens, and pricing rules. Session cleanup controls to abandon stale sessions or purge old data by age. Browser notification preferences with per-event toggles. A system information panel shows database row counts, file sizes, server uptime, and WebSocket connection status at a glance.":
      "모든 Claude 변형에 대해 토큰당 요율을 설정할 수 있는 모델 가격 편집기입니다. 수집 시 개인정보 마스킹이 이벤트 페이로드의 비밀값을 SQLite 기록 전에 가립니다 (Settings → Privacy, 전/후 미리보기 포함). 원클릭 재설치와 훅별 상태 점검이 가능한 훅 설치 상태를 제공합니다. 세션, 에이전트, 이벤트, 토큰, 가격 규칙을 아우르는 전체 JSON 데이터 내보내기가 가능합니다. 오래된 세션을 중단시키거나 기간별로 오래된 데이터를 삭제하는 세션 정리 기능도 있습니다. 이벤트별로 켜고 끌 수 있는 브라우저 알림 설정을 제공합니다. 시스템 정보 패널은 데이터베이스 행 수, 파일 크기, 서버 가동 시간, WebSocket 연결 상태를 한눈에 보여줍니다.",
    "Local MCP sidecar with three transport modes — stdio for Claude Code native integration, HTTP+SSE for remote clients, and an interactive REPL for ad-hoc terminal queries. Exposes 25 typed tools across 6 domains: sessions, agents, events, analytics, settings, and system health. Every mutation is gated behind a tiered policy so nothing dangerous fires without opt-in. Retry-aware API access handles transient failures. Runs as a standalone Node process with no Docker or cloud dependency.":
    ...
    '<span class="caption-icon">⚙️</span> <span><strong>Settings</strong> — model pricing editor with per-token rates, ingest privacy redaction, hook installation status, JSON data export, session cleanup controls, browser notification toggles, and system info panel with DB stats</span>':
      '<span class="caption-icon">⚙️</span> <span><strong>Settings</strong> — 토큰당 요율을 설정하는 모델 가격 편집기, 수집 시 개인정보 마스킹, 훅 설치 상태, JSON 데이터 내보내기, 세션 정리 기능, 브라우저 알림 토글, DB 통계가 포함된 시스템 정보 패널</span>',
    "This chart tracks how interest in Claude Code Agent Monitor has grown over time. The curve keeps climbing as more developers discover the project, share it, and use it in real workflows. Each new star is a small vote of confidence from the community.":
