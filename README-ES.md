...
| **Paleta de comandos** | Lanzador global `Cmd/Ctrl+K` sobre páginas, sesiones y acciones rápidas. Una sola consulta resuelve tres grupos: las nueve rutas de la barra lateral, emparejadas por su etiqueta **traducida** (así funciona en todos los idiomas); búsqueda de sesiones en vivo del lado del servidor vía `/api/sessions?q=` (con debounce y respetando el ámbito de datos, así que nunca mantiene un índice obsoleto de miles de sesiones en el cliente); y una lista corta de saltos que de otro modo quedan a varios clics. También se abre desde el botón de búsqueda en la parte superior de la barra lateral, que muestra el atajo correcto según la plataforma. Totalmente manejable con el teclado — flechas, `Enter`, `Escape` — y degrada en silencio si la consulta de sesiones falla |
