...
| `DASHBOARD_CODEX_HOOK_IDLE_MINUTES` | `10` | Ventana de inactividad para retirar una sesión de Codex **solo de hooks** — una cuya ejecución nunca escribió un rollout en disco (`codex exec --ephemeral`), por lo que no tiene mtime de transcripción que pruebe que sigue viva ni rollout que la sonda de procesos pueda emparejar. `SessionEnd` normalmente la cierra de inmediato; cuando ese hook fire-and-forget se pierde (panel apagado al salir, CLI terminado) esta ventana de silencio es la única evidencia restante, y importa sobre todo en Windows, donde ninguna de las dos sondas de procesos está disponible. Una sesión que en realidad siga ejecutándose se autorrepara en su siguiente hook |
