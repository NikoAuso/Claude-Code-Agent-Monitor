...
| `DASHBOARD_SSE_MAX_CLIENTS` | `50` | Número máximo de clientes Server-Sent Events simultáneos en `GET /api/events/stream`. Cada flujo abierto retiene un socket durante toda su vida, así que el límite acota lo que puede consumir un script descontrolado; las conexiones adicionales se rechazan con `503 STREAM_LIMIT` hasta que una se desconecte. Ponlo a `0` para desactivar por completo el endpoint SSE (el feed WebSocket no se ve afectado) |
